<template>
  <div
    class="min-h-screen relative bg-gradient-to-br from-green-100 via-blue-50 to-purple-100 py-6 px-3 sm:py-8 sm:px-4"
  >
    <!-- ОСНОВНОЙ БЛОК -->
    <div
      v-if="!finished"
      class="mx-auto w-full max-w-2xl bg-white rounded-2xl shadow-2xl p-4 sm:p-8 border-t-4 border-green-400"
    >
      <h2
        class="text-2xl sm:text-3xl font-bold text-center mb-2 text-green-700 break-words"
      >
        🌍 Ударения в заимствованных словах
      </h2>

      <p
        class="text-center text-gray-500 mb-6 text-sm sm:text-base leading-relaxed"
      >
      ...
      <div>
        ...
        <div
          class="mb-4 text-base sm:text-lg font-semibold text-center text-green-700 break-words leading-relaxed"
        >
        ...
        <div v-if="!blockFinished">
          <div
            class="text-center text-lg sm:text-xl mb-4 leading-relaxed break-words [overflow-wrap:anywhere]"
          ...
          <div class="relative mb-3">
            <input
              ...
              class="w-full min-w-0 px-4 py-3 pr-16 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-400 text-base sm:text-lg"
            />
          ...
          <button
            v-else
            @click="checkAnswer"
            class="w-full bg-green-500 hover:bg-green-600 text-white font-bold py-3 px-4 rounded-xl transition"
          >
            Проверить
          </button>
          ...
          <div class="flex justify-between gap-3 mt-6">
            <button
              ...
              class="flex-1 min-w-0 px-3 sm:px-6 py-3 rounded-xl font-semibold border border-gray-300 bg-white hover:bg-gray-100 disabled:opacity-40 text-sm sm:text-base transition"
            >
              ⬅ Назад
            </button>
        ...
        <div
          v-else
          class="text-center mt-6 bg-green-50 p-4 sm:p-6 rounded-xl border border-green-200"
        >
          <p class="text-xl text-green-700 font-semibold mb-2">
            📘 Подсказка:
          </p>

          <p class="text-gray-700 italic leading-relaxed break-words">
            {{ currentBlock.rule }}
          </p>

          <button
            @click="nextBlock"
            class="mt-4 w-full sm:w-auto bg-purple-600 hover:bg-purple-700 text-white font-bold py-3 px-6 rounded-xl transition"
          >
            Далее
          </button>
        </div>
    ...
    <div
      v-if="!finished"
      class="mx-auto mt-4 w-full max-w-2xl bg-white shadow-xl rounded-xl p-3 sm:p-4 border border-green-300 md:absolute md:top-6 md:right-6 md:mt-0 md:w-72"
    >
      <h3 class="text-center font-bold mb-3 text-green-700">
        Вопросы
      </h3>
      ...
      <div class="grid grid-cols-5 gap-2">
        <button
          ...
          :key="idx"
          @click="jumpTo(idx)"
          class="w-full h-10 sm:h-11 rounded-md font-semibold flex items-center justify-center border transition text-sm sm:text-base"
          :class="{
    ...
    >
      <div class="text-center mt-2 sm:mt-6">
        <div
          class="bg-white rounded-xl shadow-lg p-5 sm:p-6 inline-block w-full"
        >
          <p
            class="text-2xl sm:text-3xl font-bold text-green-700 mb-2"
          >
            🎉 Ваши результаты
          </p>
          ...
          <p
            class="text-3xl sm:text-4xl font-extrabold text-green-600 mb-2"
          >
...
const blocks: Block[] = [
  {
    ...
    questions: [
      {
        sentence: "Портмоне лежало на столе.",
        correct: "портмонЕ"
      },
      {
        sentence: "Платье из ткани гофре.",
        correct: "гофрЕ"
      },
      {
        sentence: "Выступал квартет.",
        correct: "квартЕт"
      },
      {
        sentence: "Поезда стояли в депо.",
        correct: "депО"
      },
      {
        sentence: "Она опустила жалюзи.",
        correct: "жалюзИ"
      }
    ]
  },

  {
    ...
    questions: [
      {
        sentence: "Менеджмент — это наука.",
        correct: "мЕнеджмент"
      },
      {
        sentence: "Команда прошла тренинг.",
        correct: "трЕнинг"
      },
      {
        sentence: "Учитель включил таймер.",
        correct: "тАймер"
      },
      {
        sentence: "Маркетинг важен.",
        correct: "мАркетинг"
      },
      {
        sentence: "Ошибка на сервере.",
        correct: "сЕрвере"
      }
    ]
  },

  {
    ...
    questions: [
      {
        sentence: "Алюминиевые рамы.",
        correct: "алюмИниевые"
      },
      {
        sentence: "Вставьте апостроф.",
        correct: "апострОф"
      },
      {
        sentence: "Учёный выдвинул гипотезу.",
        correct: "гипОтезу"
      },
      {
        sentence: "Школьный бюллетень.",
        correct: "бюллетЕнь"
      },
      {
        sentence: "Он пошёл в стоматологию.",
...
const formattedSentence = computed(() => {
  ...
  const reg = new RegExp(cleanWord, "i")
...
const percent = computed(() =>
  Math.round(
    (correctCount.value / totalQuestions) * 100
  )
)
